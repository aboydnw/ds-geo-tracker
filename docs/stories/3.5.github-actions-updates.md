...
## Tasks / Subtasks
- [x] Update workflow timeout (AC: 1)
  - [x] Change `timeout-minutes: 2` to `timeout-minutes: 5`
- [x] Add LLM API key secrets (AC: 2, 3)
  - [x] Add `PERPLEXITY_API_KEY: ${{ secrets.PERPLEXITY_API_KEY }}`
  - [x] Add `OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}`
  - [x] Add `ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}`
  - [x] Add `GOOGLE_AI_API_KEY: ${{ secrets.GOOGLE_AI_API_KEY }}`
  - [x] All use `${{ secrets.* }}` which resolve to empty string if not configured — this is fine, sources check for presence
- [x] Update Plausible domain secret (AC: 4)
  - [x] Document that `PLAUSIBLE_DOMAIN` should now be set to the dedicated tracker site (e.g., `geo.developmentseed.org`)
- [x] Add test step (AC: 5)
  - [x] Add `run: npm test` step before `run: node index.js`
  - [x] Tests run without any API keys (all unit tests use mocks)
...
- All 4 LLM API key secrets added (done incrementally during Stories 3.2/3.3)
- Test step (`npm test`) added before tracker run — 86 tests pass
- Fixed npm test glob to properly include `src/sources/*.test.js`
- Plausible domain already configured as dedicated tracker site

### File List
| File | Action | Description |
|------|--------|-------------|
| `.github/workflows/geo-tracker.yml` | Modified | Timeout 5min, test step, all secrets |
| `package.json` | Modified | Fixed test glob to include nested test files |

## Change Log
| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2026-02-08 | 0.1 | Story created for GitHub Actions workflow updates | Review |
| 2026-02-08 | 1.0 | Implementation complete — timeout, test step, all secrets, fixed test glob | Dev Agent |
